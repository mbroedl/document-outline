...
class OutlineTreeView {
  ...
  render() {
    ...
    if (this.plainText.trim().endsWith('}')) {
    	let re = /{[^{}]*}$/
    	let iconTitle = this.plainText.match(re)
    	extraIcon = <span class="icon icon-tag" style="margin-left: 0.5em;" title={iconTitle}></span>
    	this.plainText = this.plainText.replace(re, '')
    }
